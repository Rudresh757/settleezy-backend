...
async function submitStudentAmbassadorApplication(req, res) {
  try {
    ...
    if (!body.eligEnrolled || !body.eligSixMonths || !body.eligContent || !body.eligVoluntary || !body.consentGdpr) {
      return res.status(400).json({
        success: false,
        message: 'All required eligibility and GDPR consent checkboxes must be accepted.',
      })
    }

    const ref = generateRef()
    const doc = new StudentAmbassadorApplication({
      ...body,
      referenceNumber: ref,
    ...
    const mailErrors = await sendFormEmails({
      formLabel: 'Student Ambassador Application',
      ref,
      ...
      fields: [
        ...
        ['Time in Germany', body.germanyDuration],
        ['Primary Platform', body.platform],
        ['Handle', body.handle],
        ['Follower Count', body.followers],
        ['Secondary Platform', body.secondaryPlatform],
        ['Motivation', body.motivation],
        ['Student Story', body.story],
        ['Campus Approach', body.approach],
    ...
    return res.status(201).json({
      success: true,
      message: 'Student ambassador application submitted successfully.',
      ref,
    })
  } catch (err) {
...
async function downloadStudentAmbassadorApplicationsCSV(req, res) {
  try {
    ...
    if (!docs.length) {
      return res.status(404).json({ success: false, message: 'No student ambassador applications to export.' })
    }
