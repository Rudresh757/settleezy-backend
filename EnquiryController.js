...
const submitEnquiry = async (req, res) => {
  try {
    ...
    if (!flat.fullName || !flat.workEmail) {
      return res.status(400).json({ success: false, message: 'fullName and workEmail are required.' })
    }
    ...
    //     ['Work Email', flat.workEmail], ['Phone', flat.phone],
    //     ['Business Website', flat.businessWebsite], ['Business Category', flat.businessCategory],
    //     ['Address', flat.address], ['Other Platforms', flat.platforms.join(', ')],
    //     ['Offer Description', flat.offer], ['Intro Call', flat.introCall],
    //     ['Preferred Time', flat.preferredTime], ['How Heard', flat.howHeard],
    //     ['Additional Info', flat.additionalInfo], ['Agreed to Terms', flat.agreeTerms ? 'Yes' : 'No'],
    //     ['Submitted At', new Date().toLocaleString('en-GB', { timeZone: 'Europe/Berlin' })],
    //   ],
    // })
    // if (mailErrors.length) {
    //   return res.status(502).json({ success: false, message: 'Enquiry saved, but email delivery failed.', ref, referenceNumber: ref, mailErrors })
    // }
    // ── END EMAIL BLOCK ──────────────────────────────────────────────────────
...
const downloadEnquiryCSV = async (req, res) => {
  try {
    const docs = await EnquiryForm.find().sort({ submittedAt: -1 }).lean()
    if (docs.length === 0) return res.status(404).json({ success: false, message: 'No enquiry submissions to export.' })
    ...
    const fields = [
      'referenceNumber', 'submittedAt', 'fullName', 'businessName', 'workEmail', 'phone',
      'businessWebsite', 'businessCategory', 'address', 'platforms',
      'offer', 'introCall', 'preferredTime', 'howHeard', 'additionalInfo', 'agreeTerms',
