...
const { isOldPartnerDeadlinePassed, validatePartnerPayload } = require('./Validation')
...
const submitOldPartnerApplication = async (req, res) => {
  try {
    ...
    //     ['Trading Name', formData.tradingName], ['Rep. Full Name', formData.repFullName],
    ...
    //   return res.status(502).json({ success: false, message: 'Old partner form saved, but email delivery failed.', ref, id: application._id, mailErrors })
    // }
    // ── END EMAIL BLOCK ──────────────────────────────────────────────────────
    ...
    if (err.code === 11000) return res.status(409).json({ success: false, message: 'Duplicate reference number. Please try submitting again.' })
    return res.status(500).json({ success: false, message: 'Server error. Please try again later.' })
  }
}

// GET /api/old-partner-applications
...
// GET /api/old-partner-applications/download/csv
